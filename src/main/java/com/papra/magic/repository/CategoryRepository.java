...
public interface CategoryRepository extends JpaRepository<Category, Long> {
    @Query(
        value = "select distinct category from Category category left join fetch category.actions",
    ...
    @Query("select distinct category from Category category left join fetch category.actions")
    List<Category> findAllWithEagerRelationships();

    @Query("select category from Category category left join fetch category.actions where category.id =:id")
